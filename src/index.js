...
bot.on('message', message => {
  ...
  switch (message.text) {
    ...
    case keyboardButtons.film.random:
      ...
      break
    case keyboardButtons.home.favorite:
      showFavouriteFilms(chatId, message.from.id)
      break
  ...
  if (message.location) {
    getCinemasInCoord(chatId, message.location)
  }
})
...
bot.onText(/\/start/, message => {
  const name = helper.getChatFirstName(message)
  const hello = `Здраствуйте, ${name}\nВыберите команду для начала работы:`
...
bot.onText(/\/f(.+)/, (message, [source, match]) => {
  ...
  Promise.all([
    ...
    User.findOne({telegramId: message.from.id})
  ]).then(([film, user]) => {
...
bot.onText(/\/c(.+)/, (message, [source, match]) => {
...
function getCinemasInCoord(chatId, location) {
  ...
  Cinema.find({}).then(cinemas => {
    ...
    const html = cinemas.map((c, i) => {
      return `<b>${i + 1}</b> ${c.name}. <em>Расположение</em> - <strong>${c.distance}</strong> км. /c${c.uuid}`
    }).join('\n')
...
function toggleFavoriteFilm(userId, queryId, {filmUuid, isFav}) {
  ...
  .then(user => {
    ...
    const answerText = isFav ? 'Удалено' : 'Добавленно'
